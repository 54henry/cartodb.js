...
MapboxGeocoder.geocode = function (address, callback) {
  ...
  var token = 'pk.eyJ1IjoiY2FydG8tdGVhbSIsImEiOiJjamQwYW00bXIwcnJoMzFwNG01eWV5NHFqIn0.jcDAWcqrdCu8nTjicM4VHA';
  var url = ENDPOINT.replace('{{address}}', address).replace('{{access_token}}', token);
  ...
  $.ajax({
    ...
    error: function (response) {
      ...
      console.error('[Mapbox geocoder error] ' + message);
    }
  });
};
